"""RunsMixin: the model-execution capabilities of :class:`LabSDK`.

Split out of ``sdk.py`` to keep each file within the 150-line limit. These
methods assume the composing class provides ``self._config`` (a ``Config``).
Heavy torch/transformers/airllm imports stay deferred to call time.
"""
...
class RunsMixin:
    """Baseline / AirLLM / benchmark execution methods for the SDK facade."""
